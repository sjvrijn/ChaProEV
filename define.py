...
class Leg:
    ...
    def __init__(self, name, parameters_file_name):
        self.name = name

        parameters = cook.parameters_from_TOML(parameters_file_name)
        leg_parameters = parameters['legs'][name]
        self.distance = leg_parameters['distance']
        self.duration = leg_parameters['duration']
        self.hour_in_day_factors = leg_parameters['hour_in_day_factors']
        locations = leg_parameters['locations']
        self.start_location = locations['start']
        self.end_location = locations['end']
        road_type_parameters = leg_parameters['road_type_mix']
        self.road_type_mix = {
            road_type: road_type_parameters[road_type]
...
class Vehicle:
    ...
    def __init__(self, name, parameters_file_name):
        ...
        self.base_consumption = vehicle_parameters['base_consumption']
        self.battery_capacity = vehicle_parameters['battery_capacity']
        self.solar_panel_size_kWp = vehicle_parameters['solar_panel_size_kWp']

        road_factor_parameters = vehicle_parameters['road_factors']
        self.road_factors = {
            road_type: road_factor_parameters[road_type]
...
class Location:
    ...
    def __init__(self, name, parameters_file_name):
        ...
        self.connectivity = location_parameters['connectivity']
        self.charging_power = location_parameters['charging_power']
        self.latitude = location_parameters['latitude']
        self.longitude = location_parameters['longitude']
        self.base_charging_price = location_parameters['base_charging_price']
        self.charging_desirability = location_parameters['charging_desirability']
...
class Trip:
    ...
    def __init__(self, name, parameters_file_name):
        ...
        location_names = list(location_parameters)
        trip_parameters = parameters['trips'][name]
        self.vehicle = trip_parameters['vehicle']
        self.legs = trip_parameters['legs']
        self.percentage_station_users = trip_parameters['percentage_station_users']
        self.start_probabilities = trip_parameters['start_probabilities']
        self.day_start_hour = trip_parameters['day_start_hour']
        ...
        trip_start = datetime.datetime(2001, 1, 1, self.day_start_hour)
        trip_end = datetime.datetime(2001, 1, 2, self.day_start_hour)
        ...
        self.time_index = pd.MultiIndex.from_tuples(
            trip_time_index_tuples, name=['Hour', 'Minute', 'Second']
        )

        self.base_dataframe = pd.DataFrame(index=self.time_index)

        location_parameters = parameters['locations']
        locations = list(location_parameters.keys())

        empty_values = np.empty((len(self.time_index), len(locations)))
        empty_values[:] = np.nan
        self.base_dataframe[location_names] = empty_values
        self.located_at = self.base_dataframe.copy()
        self.connected = self.base_dataframe.copy()
        self.available_power_kW = self.base_dataframe.copy()
        self.battery_space_kWh = self.base_dataframe.copy()
        self.drawn_charge_kWh = self.base_dataframe.copy()
        self.energy_necessary_for_next_leg = self.base_dataframe.copy()
...
def declare_class_instances(Chosen_class, parameters_file_name):
    ...
    return [
        Chosen_class(class_name, parameters_file_name)
